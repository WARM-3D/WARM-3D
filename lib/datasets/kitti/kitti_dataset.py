...
from lib.datasets.utils import angles2class, class2angles, angle2class
...
class KITTI_Dataset(data.Dataset):
    def __init__(self, split, cfg):
        ...
        # basic configuration
        self.root_dir = cfg.get('source_root_dir')
        self.dataset = cfg.get('source_dataset_name', 'kitti')
        self.split = split
        self.num_classes = 3
        self.max_objs = 50
        ...
        if self.dataset == 'carla':
            self.label_dir = os.path.join(self.data_dir, 'label_2_aligned')
    ...
    def get_extrinsics(self, idx):
        extrinsics_file = os.path.join(self.extrinsics_dir, '%06d.yaml' % idx)
    ...
    def __getitem__(self, item):
        ...
        info = {'img_id': index,
                ...
                'data_domain': 0}  # 1: target domain, 0: source domain
        ...
        for i in range(object_num):
            ...
            if objects[i].pos[-1] > threshold:
                # print('filter out ', objects[i].pos[-1], objects[i].cls_type)
                continue
            ...
            if proj_inside_img == False:
                # print('filter out ', center_3d, objects[i].cls_type)
                continue
            ...
            if l < 0 or r < 0 or t < 0 or b < 0:
                if self.clip_2d:
                    ...
                    # print('filter out ', l, r, t, b, objects[i].cls_type)
                    continue
        ...
        targets = {
            ...
            'mask_2d': mask_2d,
            'ground': ground,
            'feature_extrinsic': np.array(extrinsics.extrinsics_list)}
