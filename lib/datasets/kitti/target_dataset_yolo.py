...
from lib.datasets.utils import class2angles, angle2class
...
class TargetDataset(data.Dataset):
    def __init__(self, split, cfg):
        ...
        # basic configuration
        self.root_dir = cfg.get('target_root_dir')
        self.dataset = cfg.get('target_dataset_name', 'tum')
        self.split = split
        self.max_objs = 50
        self.class_name = ["Car", "Pedestrian", "Cyclist",  'Van', 'Bus', "BigCar"]
        ...
        # data split loading
        if "s110_o" in self.root_dir:
            view_ext = cfg.get('s110_s_extrinsic', None)
        elif "s110_w" in self.root_dir:
            view_ext = cfg.get('s110_w_extrinsic', None)
        elif "s110_n" in self.root_dir:
            view_ext = cfg.get('s110_n_extrinsic', None)
        elif "s110_s" in self.root_dir:
            view_ext = cfg.get("s110_s_extrinsic", None)
        else:
            raise ValueError('Unknown view')
        
        rotation_matrix  = np.array(view_ext).reshape(4,4)[:3,:3]
        ...
        self.view_ext = np.array([yaw_ground, pitch_ground, roll_ground],dtype=np.float32)

        assert self.split in ['train', 'val', 'trainval', 'test']
        # self.split_file = os.path.join(
            # self.root_dir, 'ImageSets_yolo9', self.split + '.txt')
        self.split_file = os.path.join(
        ...
        # self.label_dir = os.path.join(self.data_dir, 'label_2_yolo_v9')
    ...
    def eval(self, results_dir, logger):
        ...
        gt_annos = kitti.get_label_annos(self.gt_label_dir, img_ids)
    ...
    def __getitem__(self, item):
        ...
        img_weak_augmented = np.array(img_weak_augmented).astype(np.float32) / 255.0
        img_weak_augmented = img_weak_augmented[:, :, :3]
        img_weak_augmented = (img_weak_augmented - self.mean) / self.std
        img_weak_augmented = img_weak_augmented.transpose(2, 0, 1)  # C * H * W
        ...
        for i in range(object_num):
            ...
            if objects[i].cls_type not in self.cls2id.keys():
                ...
                raise ValueError('Unknown class type', objects[i].cls_type)

            obj_angles = np.array(objects[i].ry)
            ...
            if obj_angles > np.pi:
                obj_angles -= 2 * np.pi
            elif obj_angles < -np.pi:
                obj_angles += 2 * np.pi
            # heading_bin[i, :], heading_res[i, :] = angles2class(obj_angles)
            heading_bin[i], heading_res[i] = angle2class(obj_angles)
        ...
        info = {'img_id': index,
                ...
                'data_domain': 1,
                'view': self.view_ext,
                'img_weak_augmented': img_weak_augmented}  # 1: target domain, 0: source domain
...
if __name__ == '__main__':
    ...
    cfg = {
        'target_root_dir': '/data/tum_traffic_intersection_dataset_aligned',
        'random_flip': 0.5,
        'random_crop': 0.5,
        'scale': 0.05,
        'shift': 0.05,
        'use_dontcare': False,
        'class_merging': False,
        'writelist': ['Pedestrian', 'Car', 'Cyclist', 'Van', 'Truck', 'Bus', 'Motorcycle', 'Trailer'],
        'use_3d_center': True,
        'bbox2d_type': 'anno',
        'meanshape': False,
        'aug_pd': True,
        'aug_crop': True,
        'depth_scale': 'normal',
        'target_dataset_name': 'tum',
    }
    dataset = TargetDataset('train', cfg)
    ...
    for batch_idx, (inputs, calib, targets, info) in enumerate(dataloader):
        ...
        center_2d_norm = [boxes[:, 0], boxes[:, 1]]  # Assuming center_2d_norm is at index 1 and 2
        size_2d_norm = [boxes[:, 2], boxes[:, 3]]  # Assuming size_2d_norm is at index 3 and 4
        boxes_xyxy = box_ops.box_cxcywh_to_xyxy(boxes)
        boxes_xyxy = boxes_xyxy * torch.tensor([960, 600, 960, 600], dtype=boxes_xyxy.dtype)

        boxes_3d = targets['boxes_3d'][0]
        center_3d_norm = [boxes_3d[:, 0], boxes_3d[:, 1]]  # Assuming center_3d_norm is at index 1 and 2
        l, r, t, b = boxes_3d[:, 2], boxes_3d[:, 3], boxes_3d[:, 4], boxes_3d[:, 5]

        w, h = img.size
        center_2d = ((center_2d_norm[0] * w).to(torch.int), (center_2d_norm[1] * h).to(torch.int))
        center_3d = ((center_3d_norm[0] * w).to(torch.int), (center_3d_norm[1] * h).to(torch.int))

        heading_bin = targets['heading_bin'][0]
        heading_res = targets['heading_res'][0]
        angle = class2angles(heading_bin, heading_res, to_label_format=True)

        # Draw 2D and 3D centers on the image
        img_array = np.array(img)
        for i in range(center_2d[0].shape[0]):
            cv2.circle(img_array, (center_2d[0][i].item(), center_2d[1][i].item()), 5, (0, 255, 0), -1)

        for i in range(center_3d[0].shape[0]):
            cv2.circle(img_array, (center_3d[0][i].item(), center_3d[1][i].item()), 5, (0, 0, 255), -1)

        for j, box in enumerate(boxes_xyxy):
            cv2.rectangle(img_array, (int(box[0]), int(box[1])), (int(box[2]), int(box[3])), (255, 0, 0), 2)
            cv2.putText(img_array, f"{j}", (int(box[0]), int(box[1] - 10)), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (255, 0, 0),
                        2)
        # Save the image with centers
        cv2.imwrite('output_image.png', cv2.cvtColor(img_array, cv2.COLOR_RGB2BGR))
    ...
    for object in objects:
        print(object.to_kitti_plus_format())
