...
sys.path.append('/workspace/lib/datasets/dataset-dev-kit')
...
superclass_iou_threshold_dict = {
    "VEHICLE": 0.1, "PEDESTRIAN": 0.1, "BICYCLE": 0.1}  # 0.7  # 0.3  # 0.5
...
def get_evaluation_results(
    ...
    # assert len(gt_annotation_frames) == len(
    #     pred_annotation_frames), "the number of GT must match predictions"
    assert difficulty_mode in ["EASY", "MODERATE", "HARD",
                               "OVERALL"], "difficulty mode is not supported"
    ...
    ious = compute_iou3d_cpu(
        gt_annotation_frames, pred_annotation_frames, prediction_type=prediction_type)
    ...
    for sample_idx in tqdm(range(num_samples)):
        ...
        if use_superclass:
            if gt_anno["name"].size > 0:
                ...
                n_bicylces = (np.logical_or(
                    gt_anno["name"] == "BICYCLE", gt_anno["name"] == "MOTORCYCLE")).sum()
            ...
            if pred_anno["name"].size > 0:
                ...
                n_bicylces = (np.logical_or(
                    pred_anno["name"] == "BICYCLE", pred_anno["name"] == "MOTORCYCLE")).sum()
                n_vehicles = len(pred_anno["name"]) - \
                    n_pedestrians - n_bicylces
                for obj_class in classes:
                    if obj_class.upper() == "VEHICLE":
            ...
            for cur_class in classes:
                if gt_anno["name"].size > 0:
                    gt_class_occurrence[cur_class] += (
                        gt_anno["name"] == cur_class.upper()).sum()
                if pred_anno["name"].size > 0:
                    pred_class_occurrence[cur_class] += (
                        pred_anno["name"] == cur_class.upper()).sum()
    ...
    for cls_idx, cur_class in enumerate(classes):
        ...
        for diff_idx in range(num_difficulties):
            ...
            iou_3d[cls_idx, diff_idx] = np.average(
                all_ious) if len(all_ious) else 0.0
            pos_err[cls_idx, diff_idx] = np.average(
                all_pos) if len(all_pos) else 0.0
            rot_err[cls_idx, diff_idx] = np.average(
                all_rot) if len(all_rot) else 0.0

            ### compute tp/fp/fn ###
            confusion_matrix = np.zeros(
                [len(thresholds), 3])  # only record tp/fp/fn
            for sample_idx in range(num_samples):
    ...
    for cls_idx, cur_class in enumerate(classes):
        ...
        ret_str += "%-20s|" % (str(pred_class_occurrence[cur_class]) + "/" + str(
            gt_class_occurrence[cur_class]))
        ...
        if use_superclass:
            if class_name == "VEHICLE":
                reject = np.logical_or(
                    ...
                    np.logical_or(gt_anno["name"] == "BICYCLE",
                                  gt_anno["name"] == "MOTORCYCLE"),
                )
            elif class_name == "BICYCLE":
                reject = ~np.logical_or(
                    gt_anno["name"] == "BICYCLE", gt_anno["name"] == "MOTORCYCLE")
            else:
        ...
        if use_superclass:
            if class_name == "VEHICLE":
                reject = np.logical_or(
                    ...
                    np.logical_or(
                        pred_anno["name"] == "BICYCLE", pred_anno["name"] == "MOTORCYCLE"),
                )
            elif class_name == "BICYCLE":
                reject = ~np.logical_or(
                    pred_anno["name"] == "BICYCLE", pred_anno["name"] == "MOTORCYCLE")
...
def iou3d_kernel(gt_boxes, pred_boxes):
    ...
    Returns:
        iou3d: [N, M]
    """
    intersection_2d = rotate_iou_gpu_eval(
        gt_boxes[:, [0, 1, 3, 4, 6]], pred_boxes[:, [0, 1, 3, 4, 6]], criterion=2)
...
def compute_iou3d(gt_annos, pred_annos, split_parts, with_heading):
    ...
    pred_num_per_sample = np.stack(
        [len(anno["name"]) for anno in pred_annos], 0)
    ious = []
    sample_idx = 0
    for num_part_samples in split_parts:
        ...
        gt_boxes = np.concatenate([anno["boxes_3d"]
                                   for anno in gt_annos_part], 0)
        pred_boxes = np.concatenate([anno["boxes_3d"]
                                     for anno in pred_annos_part], 0)
        ...
        for idx in range(num_part_samples):
            ...
            ious.append(iou3d_part[gt_num_idx: gt_num_idx +
                                   gt_box_num, pred_num_idx: pred_num_idx + pred_box_num])
        ...
        if "openlabel" in json_data:
            for frame_id, frame_obj in json_data["openlabel"]["frames"].items():
                ...
                for object_id, label in frame_obj["objects"].items():
                    ...
                    rotation = R.from_quat(
                        [quat_x, quat_y, quat_z, quat_w]).apply([l / 2.0, 0, 0])
                    position_3d = [
                        float(label["object_data"]["cuboid"]["val"][0]),
                        float(label["object_data"]["cuboid"]["val"][1]),
                        # - h / 2  # To avoid floating bounding boxes
                    ...
                    image_pos = perspective.project_from_lidar_south_to_image(
                        np.array(
                            [
                                ...
                                [position_3d[2] - h / 2,
                                 position_3d[2] - h / 2 + rotation[2]],
                            ]
                        )
                    )

                    # 3d position in s110_base with z=0
                    position_3d_in_s110_base = perspective.project_to_ground(
                        image_pos)
                    orientation_vec = position_3d_in_s110_base[:,
                                                               0] - position_3d_in_s110_base[:, 1]
                    # yaw rotation in s110_base frame
                    rotation_yaw = np.arctan2(
                    ...
                    # Category mapping
                    if label["object_data"]["type"].upper() in category_mapping:
                        label["object_data"]["type"] = category_mapping[label["object_data"]["type"]]
            ...
            for label in json_data["labels"]:
                if "dimensions" in label:
                    ...
                    rotation = R.from_quat([quat_x, quat_y, quat_z, quat_w]).as_euler(
                        "zyx", degrees=False)[0]
                    position_3d = [
                        float(label["center"]["x"]),
                        float(label["center"]["y"]),
                        # To avoid floating bounding boxes
                        float(label["center"]["z"]) - h / 2,
                    ]
                else:
                    ...
                    rotation = float(
                        label["box3d"]["orientation"]["rotationYaw"])
                    position_3d = [
                ...
                num_points = 0
                append_object(num_points, l, w, h, rotation,
                              position_3d, label["category"])
...
def prepare_predictions_kitti(predictions_path, prediction_type='mono3d', scene='day'):
    if os.path.isfile(predictions_path):
        predictions_file_list = [predictions_path]
    else:
        predictions_file_list = sorted(
            glob.glob(os.path.join(predictions_path, "*.txt")))
        if scene == 'day':
            predictions_file_list = predictions_file_list[:177]
        elif scene == 'night':
            predictions_file_list = predictions_file_list[177:]
    predictions_list = []

    for prediction_file in tqdm(predictions_file_list, desc="Loading predictions"):
        name = []
        boxes_3d = []
        scores = []
        with open(prediction_file, "r") as f:
            for line in f:
                ...
                yaw_radian = prediction[13]
                pitch_radian, roll_radian = prediction[14], prediction[15]
                rotation = np.array([0, 0, yaw_radian])
                h = prediction[7]
                l = prediction[9]
                score = prediction[-1]
                score = 0.9
                if prediction_type == "mono3d":
                    agent_pose_in_lidar_position, quat = perspective.transform_from_camera_to_lidar(
                        position_3d, roll_radian, pitch_radian, yaw_radian)

                    position_3d = agent_pose_in_lidar_position
                    rotation = R.from_quat(quat).apply([l / 2.0, 0, 0])
                    # rotation = np.array([0, 0, yaw_radian])
                ...
                if str(prediction[0]).upper() == 'CYCLIST':
                    prediction[0] = 'BICYCLE'

                # Category mapping
                if prediction[0].upper() in category_mapping:
                    prediction[0] = category_mapping[prediction[0].upper()]
                name.append(str(prediction[0]).upper())

                boxes_3d.append(
                    prediction[1:4] + [prediction[10], prediction[9], prediction[8], prediction[7]])
                scores.append(score)
        prediction_dict = {"name": np.array(
            name), "boxes_3d": np.array(boxes_3d), "score": np.array(scores), }
...
def visualize_bounding_boxes(
    ...
    gt_data = parse_input_files(
        label_path, object_min_points, ouster_lidar_only)
    # FOR KITTI
    # pred_data = prepare_predictions(prediction_path)
    # read OpenLABEL predictions/labels
    pred_data = parse_input_files(
        prediction_path, object_min_points, ouster_lidar_only)
    ...
    pc_file = os.path.join(
        get_pc_path(os.path.dirname(label_path)), rreplace(
            os.path.basename(label_path), ".json", ".pcd")
    )
    ...
    for box in gt_data[0]["boxes_3d"]:
        obb = o3d.geometry.OrientedBoundingBox(
            box[:3],
            np.array([[np.cos(box[6]), np.sin(box[6]), 0],
                      [-np.sin(box[6]), np.cos(box[6]), 0], [0, 0, 1]]),
            box[3:6],
        )
...
def parse_arguments():
    parser = argparse.ArgumentParser(description='Evaluate MonoDETR model.')
    parser.add_argument('--camera_id', type=str,
                        help='Camera ID to be used for evaluation')
    parser.add_argument('--output_dir', type=str,
                        help='Base directory for output predictions')
    parser.add_argument('--scene', type=str, default='all',
                        help='Scene to be used for evaluation')
    parser.add_argument(
        "--use_superclasses",
        action="store_true",
...
if __name__ == "__main__":
    ...
    # camera_id = 'south2'  # args.camera_id 'south1' or 'south2'
    # scene = 'all'
    # assert camera_id is not None, "Please provide the camera ID you want to use for evaluation."
    # folder_path_ground_truth = '/data/tum_traffic_intersection_dataset_' + \
    #                            camera_id + '/training/label_2'
    # # folder_path_predictions = '/data/tum_traffic_intersection_dataset_'+camera_id+'/training/label_2'
    # # folder_path_predictions = '/workspace/MonoDETR/outputs/monodetr_ema_yolov9_e21.4/outputs/' + camera_id
    # folder_path_predictions = '/workspace/MonoDETR/outputs/monodetr_bl_tum/outputs/' + camera_id
    # # folder_path_predictions = '/workspace/MonoDETR/outputs/monodetr_bl_carla_large_ext_vis_depth_MLP/outputs/' + camera_id

    args = parse_arguments()
    camera_id = args.camera_id  # Now fetched from command line arguments
    output_dir = args.output_dir  # Directory for predictions output
    scene = args.scene  # 'day' or 'night' or 'all'

    assert camera_id is not None, "Please provide the camera ID you want to use for evaluation."

    folder_path_ground_truth = '/data/tum_traffic_intersection_dataset_' + \
        camera_id + '/training/label_2'
    folder_path_predictions = output_dir

    object_min_points = 5
    use_superclasses = args.use_superclasses
    file_path_calibration_data = '/workspace/MonoDETR/lib/datasets/dataset-dev-kit/calib/s110_camera_basler_' + \
                                 camera_id + '_8mm.json'
    # possible values: [lidar3d_unsupervised, lidar3d_supervised, mono3d, multi3d]
    prediction_type = 'mono3d'
    # possible values: [openlabel, kitti]
    prediction_format = 'kitti'
    use_ouster_lidar_only = False
    perspective = parse_perspective(file_path_calibration_data)
    if camera_id == 'south1':
        perspective.transformation_matrix_from_camera_to_lidar = np.linalg.inv(np.array([
            -0.0931837,
            -0.995484,
            0.018077,
            -13.8309,
    ...
    if os.path.isfile(folder_path_ground_truth) and os.path.isfile(folder_path_predictions):
        ...
        gt_data = prepare_predictions_kitti(
            folder_path_ground_truth, prediction_type='mono3d', scene=scene)
        ...
        if prediction_format == "openlabel":
            ...
            pred_data = prepare_predictions_kitti(
                folder_path_predictions, prediction_type='mono3d', scene=scene)
        else:
            raise ValueError(
                "Unknown prediction format: {}".format(prediction_format))
