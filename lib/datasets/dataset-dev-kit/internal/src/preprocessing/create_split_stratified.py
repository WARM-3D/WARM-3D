...
# Example usage:
# Extract sampled frames from the dataset using stratified sampling among object classes
# python create_split_stratified.py split --version full --root-dir /mnt/hdd_data1/28_datasets/00_a9_dataset/01_R1_sequences/r01_sets_paper --out-dir /mnt/hdd_data1/28_datasets/00_a9_dataset/r01_full_split --balanced-class-distribution


# TODO: use scikit learn stratified shuffle built in function

# TODO: fix bug:
# The create_split_stratified.py runs without any errors but produces unequal amount of registered point clouds.
# We have 800 registered point clouds frames (640 train, 80 val, 80 test). But the output is only:
# 563 train (37 missing)
# 70 val (10 missing)
# 70 test (10 missing)

# Workaround: use file names from labels and copy registered point clouds to output folder
    ...
    for point_cloud_label in point_cloud_labels:
        ...
        for idx in label_objects:
            ...
            if label_object["object_data"]["type"] in distribution:
                distribution[label_object["object_data"]["type"]] = distribution[
                                                                        label_object["object_data"]["type"]] + 1
            else:
...
def split_to_train_val_test(point_cloud_labels, distribution, splits):
    ...
    target_len = [splits[0] * len(point_cloud_labels), splits[1] * len(point_cloud_labels),
                  splits[2] * len(point_cloud_labels)]
...
def process_set_multimodal(input_point_cloud_label_files, split_type, root_dir, out_dir, revert, replace):
    ...
    for folder in sensor_folders:
        Path(folder.replace(source_dir, out_dir + "/" + split_type + "/")).mkdir(mode=0o777, parents=True,
                                                                                 exist_ok=True)
    ...
    for input_file in input_point_cloud_label_files:
        ...
        point_cloud_filenames = label_json["openlabel"]["frames"][frame_key]["frame_properties"][
            ...
            if "point_clouds" in sensor_folder and "labels_point_clouds" not in sensor_folder:
                ...
                for point_cloud_filename in point_cloud_filenames:
                    ...
                    if sensor == point_cloud_sensor:
                        ...
                        point_cloud_full_path_out = point_cloud_full_path_in.replace(source_dir,
                                                                                     out_dir + "/" + split_type + "/")
                        assert os.path.exists(
                            point_cloud_full_path_in), "Point cloud file " + point_cloud_full_path_in + " does not exist!"
                ...
                for image_filename in image_filenames:
                    if sensor in image_filename:
                        ...
                        assert os.path.exists(
                            image_full_path_in), "Image file " + image_full_path_in + " does not exist!"
                ...
                for image_filename in image_filenames:
                    if sensor in image_filename:
                        ...
                        label_image_full_path_out = label_image_full_path_in.replace(source_dir,
                                                                                     out_dir + "/" + split_type + "/")
                        assert os.path.exists(
                            label_image_full_path_in), "Image label file " + label_image_full_path_in + " does not exist!"
        ...
        if balanced_class_distribution:
            ...
            for point_cloud_label_dir in point_cloud_label_dirs:
                ...
                for time_of_day in time_of_day_split:
                    tod_distribution, tod_distribution_pct = calculate_class_distribution(
                        time_of_day_split[time_of_day])
                    train_tod, val_tod, test_tod = split_to_train_val_test(time_of_day_split[time_of_day],
                                                                           tod_distribution, split)
...
def parse_arguments():
    ...
    parser.add_argument("--balanced-class-distribution", action="store_true",
                        help="Specify whether to try to ensure balanced class distribution between splits.")
    parser.add_argument(
        "--inplace",
        action="store_true",
...
if __name__ == "__main__":
    ...
    create_data_split_stratified(
        ...
        balanced_class_distribution=args.balanced_class_distribution,
