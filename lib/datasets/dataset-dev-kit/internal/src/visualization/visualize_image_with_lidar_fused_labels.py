...
class Utils:
    ...
    def assign_detections_to_trackers(
            self, img, trackers, detections, iou_thrd=300
    ):  # 0.3):
        """
        ...
        for t, trk in enumerate(trackers):
            for d, det in enumerate(detections):
                IOU_mat[t, d] = self.center_dist(trk, det)
        ...
        for idx_0, idx_1 in matches:
            lidar_center, camera_center = self.draw_multi_modal_centers(
                img, trackers[idx_0], detections[idx_1]
            )
            mean_center = (
            ...
            for i, corner in enumerate(trackers[idx_0]):
                trackers[idx_0][i] = np.subtract(corner, lidar_change)
            camera_change = np.subtract(camera_center, mean_center)
            for i, corner in enumerate(detections[idx_1]):
                detections[idx_1][i] = corner - camera_change
            ...
            # for t, trk in enumerate(trackers[idx_0]):
            #     for d, det in enumerate(detections[idx_1]):
            if trackers[idx_0]:
                IOU_mat = np.zeros(
                    (len(trackers[idx_0]), len(detections[idx_1])), dtype=np.float32
                )
                IOU_mat = distance.cdist(trackers[idx_0], detections[idx_1])
                ...
                for corner_idx_0, corner_idx_1 in corner_matches:
                    corner = (
                        int(
                            np.mean(
                                [
                                    trackers[idx_0][corner_idx_0][0],
                                    detections[idx_1][corner_idx_1][0],
                                ]
                            )
                        ),
                        int(
                            np.mean(
                                [
                                    trackers[idx_0][corner_idx_0][1],
                                    detections[idx_1][corner_idx_1][1],
                                ]
                            )
                        ),
                    )
        ...
        for idx_0 in unmatched_trackers:
            lidar_center, _ = self.draw_multi_modal_centers(
                img, trackers[idx_0], detections[0]
            )
            cv2.circle(
                img,
        ...
        for idx_1 in unmatched_detections:
            _, camera_center = self.draw_multi_modal_centers(
                img, trackers[0], detections[idx_1]
            )
            cv2.circle(
                img,
    ...
    # def assign_detections_to_trackers(
    #     self, img, lidar_track, detections, iou_thrd=300
    # ):  # 0.3):
    #     """
    #     From current list of trackers and new detections, output matched detections,
    #     unmatchted trackers, unmatched detections.
    ...
    #         )
    #         trackers.append(np.array(lidar_center))
    #         l.append(lidar[1])
    #         w.append(lidar[2])
    #         h.append(lidar[3])
    #         yaw.append(lidar[4])
...
if __name__ == "__main__":
    ...
    output_dir_path = root_path + f"pictures/video_frames/r01_{scenario}_{camera}_what/"
    ...
    for frame, image_file_path in enumerate(sorted(os.listdir(image_dir_path))):
        if True:
            ...
            if "openlabel" in lidar_labels:
                lidar_bbox = []
                for id, lidar_label in lidar_labels["frames"]["objects"].items():
                    category = lidar_label["object_data"]["type"]
                    color = get_color_by_category(category)
                ...
                for camera_frame in camera_labels["openlabel"]["frames"].values():
                    for camera_label in camera_frame["objects"].values():
                        ...
                        color_bgr = (0, 255, 255)
                        camera_bbox.append(
                            utils.draw_3d_box_camera_labels(
                                img, camera_label, color_bgr
                            )
                        )
                        # color_bgr = (0, 255, 0)
