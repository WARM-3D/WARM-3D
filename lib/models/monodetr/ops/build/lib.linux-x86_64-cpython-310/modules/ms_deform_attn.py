...
if float(torch.__version__.split('.')[0]) == 0 or (
    ...
    from torch.nn.modules.linear import NonDynamicallyQuantizableLinear as NonDynamicallyQuantizableLinear
...
if float(torch.__version__.split('.')[0]) == 0 or (
        float(torch.__version__.split('.')[0]) == 1 and float(torch.__version__.split('.')[1])) < 7:
        ...
        if reference_points.shape[-1] == 2:
            ...
            sampling_locations = reference_points[:, :, None, :, None, :2] \
                                 + sampling_offsets / self.n_points * (
                                         reference_points[:, :, None, :, None, 2::2] + reference_points[:, :, None,
                                                                                       :, None, 3::2]) * 0.5
        else:
            raise ValueError(
