...
def get_label_path(idx, prefix, training=True, relative_path=True):
    return get_kitti_info_path(idx, prefix, 'label_2_yolo', '.txt', training,
                               relative_path)
...
def get_label_anno(label_path):
    ...
    replace_items = ['Van', 'Truck', 'Bus', 'Trailer']
    with open(label_path, 'r') as f:
        lines = f.readlines()
    content = [line.strip().split(' ') for line in lines]
    annotations['name'] = np.array(['BigCar' if x[0] in replace_items else x[0] for x in content])
    ...
    if len(content) != 0 and len(content[0]) >= 17:
        annotations['rotation_p'] = np.array(
            [float(x[15]) for x in content]).reshape(-1)
        annotations['rotation_r'] = np.array(
            [float(x[16]) for x in content]).reshape(-1)
    if len(content) != 0 and len(content[0]) == 16:  # have score
        annotations['score'] = np.array([float(x[15]) for x in content])
    elif len(content) != 0 and len(content[0]) == 18:  # have score: p, r
