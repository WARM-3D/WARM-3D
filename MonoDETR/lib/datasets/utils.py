...
def angles2class(angles):
    ''' Convert continuous angle to discrete class and residual. '''

    # Ensure that angles are in the range [0, 2 * np.pi)
    angles = np.mod(angles, 2 * np.pi)
    assert np.all((angles >= 0) & (angles <= 2 * np.pi)), "Input angles should be in the range [0, 2 * np.pi)"

    # Calculate angle per class and shift the angles
    angle_per_class = 2 * np.pi / float(num_heading_bin)
    shifted_angles = np.mod(angles + angle_per_class / 2, 2 * np.pi)

    # Calculate class_id and residual_angle for each angle
    class_id = np.floor_divide(shifted_angles, angle_per_class).astype(int)
    residual_angle = shifted_angles - (class_id * angle_per_class + angle_per_class / 2)
...
def class2angles(cls, residual, to_label_format=False):
    ''' Inverse function to angle2class. '''
    angle_per_class = 2 * np.pi / float(num_heading_bin)
    angle_center = cls * angle_per_class
    angle = angle_center + residual
