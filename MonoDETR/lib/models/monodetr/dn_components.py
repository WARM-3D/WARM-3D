...
from utils.misc import (accuracy, inverse_sigmoid)
...
def prepare_for_dn(dn_args, tgt_weight, embedweight, batch_size, training, num_queries, num_classes, hidden_dim,
                   label_enc):
    """
    ...
    if training:
        ...
        mask_dict = {
            ...
            'known_lbs_bboxes': (known_labels, known_bboxs, known_size_3ds, known_depths, known_target_heading_cls,
                                 known_target_heading_res),
            'know_idx': know_idx,
            'pad_size': pad_size
        }
...
def dn_post_process(outputs_class, outputs_coord, outputs_3d_dim, outputs_depth, outputs_angle, mask_dict):
    ...
    if mask_dict and mask_dict['pad_size'] > 0:
        ...
        mask_dict['output_known_lbs_bboxes'] = (
            output_known_class, output_known_coord, output_known_3d_dim, output_known_depth, output_known_angle)
...
def prepare_for_loss(mask_dict):
    ...
    output_known_class, output_known_coord, output_known_3d_dim, output_known_depth, output_known_angle = mask_dict[
...
def compute_dn_loss(mask_dict, training, aux_num, focal_alpha):
    ...
    if training and 'output_known_lbs_bboxes' in mask_dict:
        known_labels, known_bboxs, known_size_3ds, known_depths, known_target_heading_cls, known_target_heading_res, \
            output_known_class, output_known_coord, output_known_3d_dim, output_known_depth, output_known_angle, num_tgt = prepare_for_loss(
            mask_dict)
        ...
        losses.update(
            tgt_loss_angles(output_known_angle[-1], known_target_heading_cls, known_target_heading_res, num_tgt))
    else:
    ...
    if aux_num:
        for i in range(aux_num):
            ...
            if training and 'output_known_lbs_bboxes' in mask_dict:
                ...
                l_dict = {k + f'_{i}': v for k, v in l_dict.items()}
                losses.update(l_dict)
                l_dict = tgt_loss_angles(output_known_angle[-1], known_target_heading_cls, known_target_heading_res,
                                         num_tgt)
                l_dict = {k + f'_{i}': v for k, v in l_dict.items()}
                losses.update(l_dict)
            else:
